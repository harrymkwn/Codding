...
string editdistance(string s1,string s2,int i,int j,vector<vector<int>> &lookup){
    ...
    else if(i>0 && (j==0 ||lookup[i-1][j] > lookup[i][j-1])) return editdistance(s1,s2,i-1,j,lookup) + "-" + s1[i-1];
    else if (j>0 && (i==0 || lookup[i-1][j] <= lookup[i][j-1]))return editdistance(s1,s2,i,j-1,lookup) + "+" + s2[j-1];
